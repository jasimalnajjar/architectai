...
/* =====================================================================
   LIVE MODE  — when the Node server (server.js) is running with an
   ANTHROPIC_API_KEY set, designs are generated by Claude in real time.
   Otherwise the app falls back to the scripted scenarios below, so the
   static build keeps working for demos with no backend.
   ===================================================================== */
let LIVE = false;          // true when designs are generated by Claude
let liveVia = null;        // "server" (key held by server.js) | "browser" (BYOK)
let LIVE_MODEL = "claude-opus-4-8";
let serverLive = false;    // /api/health reported a server-side key
let browserKey = null;     // user's key, stored only in this browser
let convo = [];            // [{role:"user"|"assistant", text}] sent to the API
const BROWSER_MODEL = "claude-opus-4-8";

async function detectLiveMode() {
  browserKey = localStorage.getItem("architectai_key") || null;
  try {
    const r = await fetch("/api/health");
    if (r.ok) {
      const h = await r.json();
      serverLive = !!(h && h.live);
      if (serverLive && h.model) LIVE_MODEL = h.model;
    }
  } catch { /* no server (static host / file://) — browser key can still drive live mode */ }
  recomputeLive();
}

function recomputeLive() {
  if (serverLive) { LIVE = true; liveVia = "server"; }
  else if (browserKey) { LIVE = true; liveVia = "browser"; LIVE_MODEL = BROWSER_MODEL; }
  else { LIVE = false; liveVia = null; }
  applyBranding();
}

function applyBranding() {
  const pills = document.querySelectorAll(".pill-demo");
  pills.forEach(p => {
    if (LIVE) { p.textContent = `Live AI · ${LIVE_MODEL}`; p.classList.add("pill-live"); }
    else { p.textContent = "Simulated AI · no backend"; p.classList.remove("pill-live"); }
  });
  const hint = document.querySelector(".chat-hint");
  if (hint) hint.textContent = LIVE
    ? `Live: designs are generated by ${LIVE_MODEL}.`
    : "Mockup: responses are scripted. Click ⚿ Connect Claude for live AI.";
  const note = document.querySelector(".hero-note");
  if (note) note.textContent = LIVE
    ? `Live AI — designs generated by ${LIVE_MODEL}`
    : "No setup · responses are scripted to demonstrate the product concept";

  const keyBtn = $("btn-key");
  if (keyBtn) {
    if (serverLive) { keyBtn.hidden = true; }
    else { keyBtn.hidden = false; keyBtn.textContent = browserKey ? "⚿ Disconnect Claude" : "⚿ Connect Claude"; }
  }
}

/* Connect/disconnect a browser-side key (BYOK). Stored in localStorage; sent
   directly to api.anthropic.com from the browser. */
function onKeyButton() {
  if (serverLive) { toast("Already live via the server."); return; }
  if (browserKey) {
    localStorage.removeItem("architectai_key");
    browserKey = null;
    recomputeLive();
    toast("Disconnected. Back to the scripted demo.");
    return;
  }
  if (!window.ArchitectDesign) {
    toast("Live module didn't load — serve over http (npm start or python -m http.server), not file://.");
    return;
  }
  const k = prompt(
    "Paste your Anthropic API key (sk-ant-...).\n\n" +
    "It is stored only in THIS browser (localStorage) and sent directly to Anthropic. " +
    "Use a key scoped for experimentation."
  );
  if (!k) return;
  const key = k.trim();
  if (!/^sk-ant-/.test(key)) { toast("That doesn't look like an Anthropic key (sk-ant-...)."); return; }
  browserKey = key;
  localStorage.setItem("architectai_key", key);
  recomputeLive();
  toast("Connected to Claude. Describe a system to design it live.");
}

/* Browser-direct Messages API call (BYOK). Returns the forced tool's input. */
async function callClaudeDirect(systemText, tool, toolName, messages, maxTokens) {
  ...
  if (!block) throw new Error("The model returned no structured output.");
  return block.input;
}

/* Mirror of server.js buildMessages() for the browser-direct path. */
function buildDesignMessages(history, message, spec) {
  const messages = [];
  ...
  if (spec && spec.states) {
    const compact = JSON.stringify({ title: spec.title, scenario: spec.scenario, states: spec.states, doc: spec.doc });
    content =
...
function metaFor(node) {
  ...
  const m = node.meta || {}; // LLM may attach per-node spec/cost/IaC for custom components
  const fb = KIND_META[node.kind] || KIND_META.app;
  return {
    desc: m.desc || cat.desc || fb.desc,
    specs: m.specs || cat.specs || fb.specs,
    cost: "cost" in m ? m.cost : ("cost" in cat ? cat.cost : fb.cost),
    tf: m.tf || cat.tf || null,
    cfn: m.cfn || cat.cfn || null,
  };
}
...
async function handleInput(text) {
  ...
  // live AI path — Claude emits the spec; renderer is unchanged
  if (LIVE) {
    await liveDesign(text, body, textEl);
    showFollowups();
    done();
    return;
  }
...
/* Call the live backend; render the design or report the error inline. */
async function liveDesign(text, body, textEl) {
  const steps = activeSpec
    ? ["Reviewing current design", "Reasoning about the change", "Updating spec, diagram & doc"]
    : ["Parsing the brief", "Selecting components & tiers", "Designing the architecture", "Drafting diagram, doc & build pack"];
  const stepsDone = runSteps(body, steps);

  let result;
  try {
    if (liveVia === "browser") {
      const AD = window.ArchitectDesign;
      if (!AD) throw new Error("Live module not loaded (serve over http, not file://).");
      const messages = buildDesignMessages(convo, text, activeSpec);
      const input = await callClaudeDirect(AD.SYSTEM_PROMPT, AD.EMIT_DESIGN_TOOL, "emit_design", messages, 16000);
      result = AD.normalizeSpec(input);
    } else {
      const res = await fetch("/api/design", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ history: convo, message: text, spec: activeSpec || null }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data && data.message ? data.message : `Request failed (${res.status})`);
      result = { reply: data.reply, spec: data.spec };
    }
  } catch (err) {
    await stepsDone;
    await typeOut(textEl, `⚠ Live AI request failed: ${err.message}\n\nCheck your key / server and try again.`);
    return;
  }

  await stepsDone;
  result.spec._version = activeSpec ? (activeSpec._version || 1) + 1 : 1;
  activeSpec = result.spec;
  activeState = "target";
  renderAll();
  await typeOut(textEl, result.reply);
...
let appcodeFile = 0;       // index of the file shown in the App Code tab
let codeBusy = false;
...
function renderBuild() {
  ...
  // App Code is special: LLM-generated files with their own file switcher.
  if (activeArtifact === "appcode") {
    if (buildActions) buildActions.style.display = "none";
    $("build-host").hidden = true;
    $("backlog-host").hidden = true;
    $("appcode-host").hidden = false;
    $("build-note").textContent = LIVE
      ? "A runnable starter codebase generated by Claude from this design."
      : "App Code needs live mode — click ⚿ Connect Claude (or run the server with a key).";
    renderAppCode();
    return;
  }
  if (buildActions) buildActions.style.display = "";
  $("appcode-host").hidden = true;
...
/* ----- App Code (LLM-generated runnable starter) ----- */

function renderAppCode() {
  const code = activeSpec && activeSpec._code;
  const empty = $("appcode-empty");
  const view = $("appcode-view");
  const genBtn = $("btn-gen-code");

  if (!code || !Array.isArray(code.files) || !code.files.length) {
    view.hidden = true;
    empty.hidden = false;
    if (genBtn) { genBtn.disabled = !LIVE || codeBusy; genBtn.textContent = codeBusy ? "Generating…" : "⚙ Generate application code"; }
    $("appcode-hint").textContent = LIVE
      ? "Claude generates a runnable starter codebase for this design — service entrypoint, API handlers, schema, Dockerfile, and a README."
      : "App Code needs live mode. Click ⚿ Connect Claude (top right) to paste your key, or run the server with a key set.";
    return;
  }

  empty.hidden = true;
  view.hidden = false;
  if (appcodeFile >= code.files.length) appcodeFile = 0;
  const sel = $("appcode-select");
  sel.innerHTML = code.files.map((f, i) => `<option value="${i}">${esc(f.path)}</option>`).join("");
  sel.value = String(appcodeFile);
  $("appcode-code").textContent = code.files[appcodeFile].content;
}

async function generateCode() {
  if (codeBusy) return;
  if (!LIVE) { toast("Connect Claude first (⚿ Connect Claude)."); return; }
  if (!activeSpec) { toast("Describe a system first."); return; }

  codeBusy = true;
  renderAppCode();
  toast("Generating application code… this can take a minute.");
  ...
  try {
    ...
    if (liveVia === "browser") {
      ...
      const messages = [{
        role: "user",
        content: "Generate the application code build-out for this design.\n```json\n" +
          JSON.stringify({ title: activeSpec.title, scenario: activeSpec.scenario, states: activeSpec.states, doc: activeSpec.doc }) +
          "\n```",
      }];
      const input = await callClaudeDirect(AD.CODE_SYSTEM_PROMPT, AD.EMIT_CODE_TOOL, "emit_code", messages, 16000);
      result = AD.normalizeCode(input);
    } else {
    ...
    if (!result.files || !result.files.length) throw new Error("No files were returned.");
    activeSpec._code = result;
    appcodeFile = 0;
    toast(`Generated ${result.files.length} files.`);
  } catch (err) {
    toast("Code generation failed: " + err.message);
  } finally {
    codeBusy = false;
    if (activeArtifact === "appcode") renderAppCode();
  }
}
...
$("btn-copy-artifact").addEventListener("click", async () => {
  ...
  if (!art) return;
  try {
    await navigator.clipboard.writeText(art.text(activeSpec));
    toast(art.file + " copied to clipboard");
  } catch { toast("Clipboard unavailable"); }
});

// App Code controls
$("btn-key").addEventListener("click", onKeyButton);
$("btn-gen-code").addEventListener("click", generateCode);
$("btn-regen-code").addEventListener("click", generateCode);
$("appcode-select").addEventListener("change", e => {
  if (!activeSpec || !activeSpec._code) return;
  appcodeFile = Number(e.target.value) || 0;
  $("appcode-code").textContent = activeSpec._code.files[appcodeFile].content;
});
$("btn-copy-code").addEventListener("click", async () => {
  try { await navigator.clipboard.writeText($("appcode-code").textContent); toast("File copied"); }
  catch { toast("Clipboard unavailable"); }
});
$("btn-dl-code").addEventListener("click", () => {
  if (!activeSpec || !activeSpec._code) return;
  const f = activeSpec._code.files[appcodeFile];
  downloadText(f.content, f.path.split("/").pop() || "file.txt", "text/plain");
});
...
function resetDesign() {
  ...
  $("appcode-host").hidden = true;
  $("appcode-view").hidden = true;
  $("appcode-empty").hidden = false;
  const ba = document.querySelector(".build-actions");
...
// probe the backend; flips the app into live (real-AI) mode if available
detectLiveMode();
