...
/* =====================================================================
   ...
   ===================================================================== */
let LIVE = false;          // flipped on by detectLiveMode() at boot
let LIVE_MODEL = null;
...
async function detectLiveMode() {
  try {
    ...
    if (h && h.live) {
      LIVE = true;
      LIVE_MODEL = h.model || "Claude";
      applyLiveBranding();
    }
  } catch { /* no server / static host — stay in scripted demo mode */ }
}

function applyLiveBranding() {
  const label = `Live AI · ${LIVE_MODEL}`;
  document.querySelectorAll(".pill-demo").forEach(p => { p.textContent = label; p.classList.add("pill-live"); });
  const hint = document.querySelector(".chat-hint");
  if (hint) hint.textContent = `Live: designs are generated by ${LIVE_MODEL}.`;
  const note = document.querySelector(".hero-note");
  if (note) note.textContent = `Live AI — designs generated by ${LIVE_MODEL}`;
}
...
async function liveDesign(text, body, textEl) {
  ...
  await stepsDone;
  data.spec._version = activeSpec ? (activeSpec._version || 1) + 1 : 1;
  activeSpec = data.spec;
  activeState = "target";
  renderAll();
  await typeOut(textEl, data.reply);

  convo.push({ role: "user", text });
  convo.push({ role: "assistant", text: data.reply });
}
